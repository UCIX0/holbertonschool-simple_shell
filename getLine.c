 ...
 * Return: bytes read
 */
ssize_t input_buf(shell_info *info, char **buf, size_t *len)
{
	ssize_t r = 0;
	size_t len_p = 0;
 ...
 * Return: r
 */
ssize_t read_buf(shell_info *info, char *buf, size_t *i)
{
	ssize_t r = 0;

	if (*i)
		return (0);
	r = read(info->input_fd, buf, READ_BUF_SIZE);
	if (r >= 0)
		*i = r;
	return (r);
}
